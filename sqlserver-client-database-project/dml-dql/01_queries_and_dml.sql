/*==================================================================================
SCRIPT 03 - DML and DQL EXAMPLES
==================================================================================*/
...
----- "Using * to select all columns from Customer ----
...
---- select specifics colums from table customer -- 
...
--------- Using Orderby clausule for order by specifics column ------------
...
------------ using orderby clause with desc to order by specific column in descending order ------------------
...
------- using order by clause for with more than one columns ----------------------------
...
--------------------------------------------------------------------------------------------------------------
----USING CLAUSES IN SELECT FOR REPORTS-----
...
--- Problem: know how many customers there are in the customer table

SELECT COUNT(Id)
  FROM Customer


--------- know the total value of the values ​​in the totalamount column for the year 2013 -----------
SELECT SUM(TotalAmount)
  FROM [Order]
 WHERE YEAR(OrderDate) = 2013


------------------ know the average of the totalamount column ----------------------
...
--- Using clause AND to combine two or more conditions

SELECT Id, FirstName, LastName, City, Country
  FROM Customer
 WHERE FirstName = 'Thomas' AND LastName = 'Hardy'

--- Using clause OR filter records when at least one of the specified conditions is true------
...
Using Clause Not to invert the result of a Boolean condition, functioning as a negation operator in English ---
...
-- Problem: List all products that are not exactly $10, $20, $30
SELECT Id, ProductName, UnitPrice
  FROM Product
 WHERE UnitPrice NOT IN (10,20,30)

-- Problem: List all orders that are  between $100 and $15000
SELECT Id, OrderDate, CustomerId, TotalAmount
  FROM [Order]
 WHERE  (TotalAmount >= 100 AND TotalAmount <= 15000)
 ORDER BY TotalAmount DESC

 -- Problem: List all orders that are not between $100 and $15000
SELECT Id, OrderDate, CustomerId, TotalAmount
  FROM [Order]
 WHERE NOT (TotalAmount >= 100 AND TotalAmount <= 15000)
 ORDER BY TotalAmount DESC

-- SQL WHERE BETWEEN---
-- Problem: List all products between $11 and $22-----
SELECT Id, ProductName, UnitPrice
  FROM Product
 WHERE UnitPrice BETWEEN 11 AND 22
 ORDER BY UnitPrice

-- Problem: List all products not between $5 and $100----
...
-----Using INNER JOIN clause in SQL to select records that have matching values ​​in both tables -----
